...
async function searchProjects(query: string, limit = 10) {
  return db
    ...
    .where(and(eq(schema.projects.isPublic, true), like(schema.projects.name, `%${query}%`)))
...
class NeonBranchManager {
  ...
  async deleteBranch(branchId: string) {
    return this.request<{ branch: Branch }>(`/projects/${this.projectId}/branches/${branchId}`, {
      method: 'DELETE',
    });
  }

  async listBranches() {
    return this.request<{ branches: Branch[] }>(`/projects/${this.projectId}/branches`);
  }
