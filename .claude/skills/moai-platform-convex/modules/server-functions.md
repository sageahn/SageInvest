...
// Run every hour
crons.interval('cleanup old drafts', { hours: 1 }, internal.documents.cleanupOldDrafts);
...
// Run every 5 minutes
crons.interval('sync external data', { minutes: 5 }, internal.sync.fetchExternalUpdates);
...
export const cleanupOldDrafts = internalMutation({
  handler: async (ctx) => {
    ...
    const oldDrafts = await ctx.db
      .query('documents')
      .withIndex('by_status_created', (q) => q.eq('status', 'draft').lt('createdAt', thirtyDaysAgo))
      .collect();
