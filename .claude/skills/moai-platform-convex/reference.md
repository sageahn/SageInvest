...
const data = useQuery(api.module.queryName, condition ? { arg: value } : 'skip');
```
...
const mutation = useMutation(api.module.mutationName).withOptimisticUpdate((localStore, args) => {
  // Update local cache
});
```
