...
function subscribeWithMetadata(docId: string, callback: (doc: DocumentWithMeta) => void) {
  return onSnapshot(doc(db, 'documents', docId), { includeMetadataChanges: true }, (snapshot) => {
    callback({
      id: snapshot.id,
      data: snapshot.data(),
      metadata: {
...
function useDocumentWithSync<T>(collectionPath: string, docId: string | null): DocumentWithSync<T> {
...
function useUserWithOrganizations(userId: string) {
  ...
  useEffect(() => {
    ...
    const unsubOrgs = onSnapshot(
      query(collection(db, 'organizations'), where('members', 'array-contains', userId)),
      (snapshot) => {
