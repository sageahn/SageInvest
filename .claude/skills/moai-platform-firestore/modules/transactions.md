...
async function demonstrateBatchOperations() {
  ...
  // Set with merge
  batch.set(doc(db, 'users', 'user-2'), { lastLogin: serverTimestamp() }, { merge: true });
...
async function getCounterTotal(counterId: string): Promise<number> {
  const shardsSnapshot = await getDocs(collection(db, 'counters', counterId, 'shards'));
...
```typescript
async function idempotentOperation(operationId: string, userId: string, amount: number) {
  await runTransaction(db, async (transaction) => {
...
```typescript
async function bulkUpdate(updates: Array<{ id: string; data: any }>, collectionPath: string) {
