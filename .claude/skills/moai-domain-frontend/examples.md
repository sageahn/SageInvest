  ...
  <div class="data-table">
    ...
    <table class="w-full">
      ...
      <tbody>
        ...
        >
          <td v-for="column in columns" :key="String(column.key)" class="px-4 py-3">
...
export function Card({ variant = 'default', children, className }: CardProps) {
  return (
    <CardContext.Provider value={{ variant }}>
      <div className={cn('rounded-lg', variantStyles[variant], className)}>{children}</div>
...
function List({ items }) {
  ...
  return items.map((item) => <Item key={item.id} id={item.id} onClick={handleClick} />);
}

// Or with proper memoization
const Item = memo(function Item({ id, onClick }: { id: string; onClick: (id: string) => void }) {
