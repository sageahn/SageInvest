...
class LazyDocumentationLoader {
  ...
  async loadDocsBatch(docIds: string[]): Promise<Map<string, DocumentContent>> {
    ...
    const topResults = await Promise.all(highPriority.map((id) => this.loadDocLazy(id, 'high')));
...
class RelatedDocumentFinder {
  ...
  private calculateSimilarity(features1: DocumentFeatures, features2: DocumentFeatures): number {
    ...
    // Tag overlap (30%)
    const tagIntersection = features1.tags.filter((t) => features2.tags.includes(t)).length;
    ...
    const lengthRatio =
      Math.min(features1.length, features2.length) / Math.max(features1.length, features2.length);
